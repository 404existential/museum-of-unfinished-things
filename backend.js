...
(() => {
  ...
  async function loadPublicArtifacts(){const {data,error}=await client.from('artifacts').select('accession,title,category,year,status,reason,visual,description,contributor_note,contributor_name,created_at').eq('status','Published').order('created_at',{ascending:false});if(error||!data?.length)return;const remote=data.map(a=>({id:a.accession,title:a.title,category:a.category,year:a.year||'Undated',status:a.status,reason:a.reason||'The circumstances were not documented.',visual:a.visual||'paper',description:a.description,note:a.contributor_note,contributor:a.contributor_name}));artifacts=[...remote,...artifacts.filter(a=>!remote.some(r=>r.id===a.id))];render()}
