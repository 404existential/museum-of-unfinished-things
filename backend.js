...
(() => {
  ...
  modal.innerHTML=`<div class="modal submit-modal" role="dialog" aria-modal="true"><button class="modal-close" id="accountClose" aria-label="Close">×</button><p class="eyebrow">Contributor archive</p><h2 id="accountHeading">Enter the museum.</h2><p class="modal-intro">Accounts are only an identity for returning to your contributions. Published artifacts remain public.</p><div id="accountBody"></div></div>`;
  ...
  async function renderAccount(){
    ...
    if(!u){
      ...
      $$('.account-artifact').forEach(b=>b.onclick=()=>{closeAccount();window.openArtifact?.(b.dataset.id)});$('#accountDeposit').onclick=()=>{closeAccount();document.querySelector('[data-open-submit]')?.click()};$('#signOut').onclick=async()=>{await client.auth.signOut();renderAccount();setAccountLabel()};
    }
  }
  async function authSubmit(e){e.preventDefault();const f=new FormData(e.currentTarget),username=String(f.get('username')).trim(),password=String(f.get('password')),mode=e.currentTarget.dataset.mode,msg=$('#authMsg');msg.textContent='';if(!/^[A-Za-z0-9_]{3,24}$/.test(username)){showMsg(msg,'Use 3–24 letters, numbers, or underscores.',true);return}const email=identityEmail(username);
    if(mode==='signup'){const {data,error}=await client.auth.signUp({email,password,options:{data:{username}}});if(error){showMsg(msg,error.message,true);return}if(data.user){showMsg(msg,`Identity created. Username: ${username}. Keep your password safe. You can now enter the archive.`);setTimeout(renderAccount,700)}}
  ...
  async function loadPublicArtifacts(){
    const {data,error}=await client.from('artifacts').select('accession,title,category,year,status,reason,visual,description,contributor_note,contributor_name,created_at').eq('status','Published').order('created_at',{ascending:false});
    if(error||!data?.length)return;
    const remote=data.map(a=>({id:a.accession,title:a.title,category:a.category,year:a.year||'Undated',status:a.status,reason:a.reason||'The circumstances were not documented.',visual:a.visual||'paper',description:a.description,note:a.contributor_note,contributor:a.contributor_name}));
    artifacts=[...remote,...artifacts.filter(a=>!remote.some(r=>r.id===a.id))]; render();
  }
  const form=$('#submitForm');
  if(form){form.onsubmit=async e=>{e.preventDefault();const u=await currentUser();if(!u){closeAccount();openAccount();return}const f=new FormData(form),username=(await client.from('profiles').select('username').eq('id',u.id).maybeSingle()).data?.username||'Anonymous',id='A—'+Date.now().toString().slice(-7),visual=['red','blue','green','yellow','paper','dark'][Math.floor(Math.random()*6)],row={accession:id,title:String(f.get('title')),category:String(f.get('category')),year:String(f.get('year')||'Undated'),last_worked_on:f.get('lastWorked')||null,status:'Published',reason:String(f.get('reason')||'The circumstances were not documented.'),visual,description:String(f.get('description')),contributor_note:String(f.get('note')||''),contributor_name:f.get('anonymous')==='on'?'Anonymous':username,owner_id:u.id};const {error}=await client.from('artifacts').insert(row);if(error){alert(error.message);return}$('#receiptNumber').textContent=id;form.hidden=true;$('#receipt').hidden=false;await loadPublicArtifacts()};}
